...
/**
 * @file cuda_hybrid_gpu.cu
 * @brief Hybrid Implementation GPU (Adaptive GPU)
 *
 * Strategy:
 * - Use Tiled Shared Mem, VT=4 for Large N .
 * - Use Tiled Warp-Per-Block, VT=4 for Small N.
 * - Dynamically switches kernel implementation based on remaining problem size.
 *
 */

// Default Thresholds
int g_large_n_threshold_gpu = 80000;

void setHybridGPUThresholds(int large_n_thresh) {
  g_large_n_threshold_gpu = large_n_thresh;
}

#define LARGE_N_THRESHOLD g_large_n_threshold_gpu
...
__device__ double device_callPayoff_hybrid_v3(double S, double K) {
  return fmax(S - K, 0.0);
}

__device__ double device_putPayoff_hybrid_v3(double S, double K) {
  return fmax(K - S, 0.0);
}

__global__ void
hybridGPU_LargeN_Kernel(const double *V_in, double *V_out, const double *u_pow,
    ...
    for (int i = 0; i < VT_V2; ++i) {
      ...
      if (local_idx < (TILE_SIZE_V2 * VT_V2) - 1 - k) {
        ...
        if (global_idx <= current_t - 1) {
          ...
          double V_exercise = isCall ? device_callPayoff_hybrid_v3(S, K)
                                     : device_putPayoff_hybrid_v3(S, K);
...
__global__ void __launch_bounds__(32)
    hybridGPU_SmallN_Kernel(const double *__restrict__ V_in,
...
__global__ void initTerminalValuesKernelHybridGPU(double *V,
                                                  const double *u_pow,
                                                  const double *d_pow,
  ...
  if (i <= N) {
    ...
    V[i] = isCall ? device_callPayoff_hybrid_v3(S, K)
                  : device_putPayoff_hybrid_v3(S, K);
  }
}
...
double priceAmericanOptionCUDAHybridGPU(const OptionParams &opt) {
  ...
  initTerminalValuesKernelHybridGPU<<<blocks, threadsPerBlock>>>(
  ...
  while (current_t > LARGE_N_THRESHOLD) {
    ...
    // Ensure we don't overshoot 0 if threshold is 0 (unlikely here)
    if (steps <= 0)
      break;
    ...
    hybridGPU_LargeN_Kernel<<<num_blocks, TILE_SIZE_V2, shared_mem_size>>>(
  ...
  // Phase 2: Small N (Warp Per Block)
  while (current_t > 0) {
    int steps = std::min(16, current_t); // Use 16 steps for V4 too

    int current_output_width = (TILE_SIZE_V4 * VT_V4) - steps;
    int num_blocks =
        (current_t + current_output_width - 1) / current_output_width;

    hybridGPU_SmallN_Kernel<<<num_blocks, TILE_SIZE_V4>>>(
        d_V_in, d_V_out, d_u_pow, d_d_pow, current_t, steps, opt.S0, opt.K,
        opt.isCall, params.discount, params.p);
    CUDA_CHECK(cudaGetLastError());
