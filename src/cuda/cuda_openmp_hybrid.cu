...
/**
 * @file cuda_openmp_hybrid.cu
 * @brief Hybrid Implementation CUDA-OpenMP (Adaptive GPU + OpenMP CPU Fallback)
 *
 * Strategy:
 * - Use Tiled Shared Mem for Large N.
 * - Use Tiled Warp-Per-Block for Medium N.
 * - Switch to CPU (OpenMP Parallel) when N is small.
 */

// Default Thresholds
int g_cpu_threshold_omp = 1000;
int g_large_n_threshold_omp = 80000;

void setHybridOpenMPThresholds(int cpu_thresh, int large_n_thresh) {
  g_cpu_threshold_omp = cpu_thresh;
  g_large_n_threshold_omp = large_n_thresh;
}

#define LARGE_N_THRESHOLD g_large_n_threshold_omp
#define CPU_THRESHOLD g_cpu_threshold_omp

// =================================================================================
// KERNEL 1: Large N (TiledV2 / Improved Kernel)
// =================================================================================
...
__device__ double device_callPayoff_hybrid_omp(double S, double K) {
  return fmax(S - K, 0.0);
}

__device__ double device_putPayoff_hybrid_omp(double S, double K) {
  return fmax(K - S, 0.0);
}

__global__ void
hybridOMP_LargeN_Kernel(const double *V_in, double *V_out, const double *u_pow,
                        const double *d_pow, int t_start, int steps, double S0,
                        double K, bool isCall, double discount, double p) {
    ...
    for (int i = 0; i < VT_V2; ++i) {
      ...
      if (local_idx < (TILE_SIZE_V2 * VT_V2) - 1 - k) {
        ...
        if (global_idx <= current_t - 1) {
          ...
          double V_exercise = isCall ? device_callPayoff_hybrid_omp(S, K)
                                     : device_putPayoff_hybrid_omp(S, K);
...
// =================================================================================
// KERNEL 2: Small N (TiledV4 / Warp-Per-Block)
// =================================================================================
...
__global__ void __launch_bounds__(32)
    hybridOMP_SmallN_Kernel(const double *__restrict__ V_in,
...
__global__ void initTerminalValuesKernelHybridOMP(double *V,
                                                  const double *u_pow,
                                                  const double *d_pow,
  ...
  if (i <= N) {
    ...
    V[i] = isCall ? device_callPayoff_hybrid_omp(S, K)
                  : device_putPayoff_hybrid_omp(S, K);
  }
}
...
double priceAmericanOptionCUDAOpenMPHybrid(const OptionParams &opt) {
  ...
  initTerminalValuesKernelHybridOMP<<<blocks, threadsPerBlock>>>(
  ...
  while (current_t > LARGE_N_THRESHOLD) {
    ...
    hybridOMP_LargeN_Kernel<<<num_blocks, TILE_SIZE_V2, shared_mem_size>>>(
  ...
  while (current_t > CPU_THRESHOLD) {
    ...
    hybridOMP_SmallN_Kernel<<<num_blocks, TILE_SIZE_V4>>>(
        d_V_in, d_V_out, d_u_pow, d_d_pow, current_t, steps, opt.S0, opt.K,
        opt.isCall, params.discount, params.p);
    CUDA_CHECK(cudaGetLastError());
  ...
  // We need two buffers on CPU for OpenMP parallelization
  std::vector<double> h_V_in(current_t + 1);
  std::vector<double> h_V_out(current_t + 1);

  CUDA_CHECK(cudaMemcpy(h_V_in.data(), d_V_in, (current_t + 1) * sizeof(double),
  ...
  // Phase 4: CPU OpenMP
  // We use the same logic as src/openmp/openmp_binomial.cpp
  for (int t = current_t - 1; t >= 0; --t) {
#pragma omp parallel for schedule(static)                                      \
    shared(h_V_in, h_V_out, h_u_pow, h_d_pow, params, t, opt)
    for (int i = 0; i <= t; ++i) {
      double S = opt.S0 * h_u_pow[i] * h_d_pow[t - i];
      double V_hold = params.discount *
                      (params.p * h_V_in[i + 1] + (1.0 - params.p) * h_V_in[i]);
      double V_exercise;
      if (opt.isCall) {
      ...
      h_V_out[i] = std::max(V_hold, V_exercise);
    }
    // Swap buffers
    std::swap(h_V_in, h_V_out);
  }
